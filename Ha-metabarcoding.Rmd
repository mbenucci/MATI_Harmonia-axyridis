...
ha18.1.post.filter[,][ha18.1.post.filter[,] <= 0.003] <- 0

ha1.clean = as.data.frame(t(ha1.clean))
ha1.clean[][ha1.clean[,]/rowSums(ha1.clean) <= 0.003] <- 0
...
box_a = ggplot(full_blk_melt,(aes(specie_type, value))) +
  ...
  theme_bw() +
  facet_grid(.~pcr) +
  theme(legend.position="NA", axis.text.x=element_blank(), axis.ticks.x=element_blank(), axis.text=element_text(size=15), axis.title=element_text(size=15), strip.text=element_text(size=15), plot.margin=unit(c(0,0.25,0,0.25), unit="cm"))
...
plotweb(web.ha, method="normal", labsize=2,  text.rot=90, y.width.low=0.1, y.width.high=0.09,	high.y=1.5, low.y=0.5, low.lab.dis=0.05, high.lablength=0, high.spacing=0.085, low.spacing=0.02, col.high="transparent",  col.interaction=c("orange","yellow","blue","red"), add=T, empty=F)
legend("bottom", legend=rownames(web.ha), bty="n", border="black",ncol=6, xpd=T)
dev.off()
```
...
png("R_images/publication_blk_vs_no-blk.png", height=4000, width=4000, units="px", res=300)
plot_grid(box_a, box_b, labels=c("a)","b)"), ncol=2, nrow=1)
dev.off()
```


```{r publication NMDS multiplot}
png("R_images/publication_NMDS_plots.png", height=4000, width=4000, units="px", res=300)
lay.matrix = matrix(c(1,1,2,2,1,1,2,2,3,3,4,5,3,3,3,0), nrow=4, ncol=4, byrow=T)
layout(lay.matrix)

plot(meta.guts,type="n", xlim=c(-1,1.5), ylim=c(-1,1.5), cex.axis=2, cex.lab=1.5)
...
ordiellipse(meta.guts,group=fact_time.g[,1],kind="ehull", lty=c(1,3),label=T,lwd=1)
text(-1,1,label="May", cex=2.5)
text(1.4,0,label="Oct", cex=2.5)
text(1.2,-0.9, label=paste0("Stress: ", round(meta.guts$stress, 4)), cex=1.5)
text(-1,1.45, label="a)", cex=3)
text(1,1.4,label="gut contents", cex=2.5)

plot(meta.comms,type="n", xlim=c(-1.5,1.5), ylim=c(-1.8,1.2), cex.axis=2, cex.lab=1.5)
...
text(-1.5,0,label="May", cex=2.5)
text(1.4,0.8,label="Oct", cex=2.5)
text(1.2,-1.72, label=paste0("Stress: ", round(meta.comms$stress, 4)), cex=1.5)
text(-1.5,1.15, label="b)", cex=3)
text(0,1.1,label="community", cex=2.5)

plot(meta.guts.comms,type="n", xlim=c(-2,5), ylim=c(-2,3), cex.axis=2, cex.lab=1.5)
...
ordiellipse(meta.guts.comms,group=fact_type[,1],kind="ehull", lty=c(1,3), label=F, lwd=2)
text(3,-1.9, label=paste0("Stress: ", round(meta.guts.comms$stress, 4)), cex=1.5)
text(-2.6,2.9, label="c)", cex=3)
text(0,2.6,label="community", cex=2.5)
text(0,-0.4,label="guts (inset)", cex=2.5)

plot(meta.guts.comms,type="n", xlim=c(-0.2,0.1), ylim=c(-0.25,0.15), xlab="", ylab="", cex.axis=2)
ordispider(meta.guts.comms,group=fact_type[,1],col=c("darkorange", "green4"),label=F, cex=1.5)
ordiellipse(meta.guts.comms,group=fact_type[,1],kind="ehull",lty=c(1,3),label=F, lwd=2)
points(meta.guts.comms, select=which(fact_county[,1]=="Oxfordshire" & fact_time[,1]=="May"),col="black", pch=1, cex=2)
points(meta.guts.comms, select=which(fact_county[,1]=="Yorkshire" & fact_time[,1]=="May"),col="blue", pch=22, cex=2)
...
name.multi=c("Aphididae","Thripidae","Acari","Tachinidae","Chironomidae","Chrysomelidae","Ectopsocidae","Psocoptera")
col.multi=c("cyan3","grey50","orchid","purple","tan4","black","darkolivegreen4","khaki")

col.prey=c("cyan3","cyan3","cyan3","grey50","grey50","orchid","cyan3","purple","cyan3","black","tan4","darkolivegreen4","cyan3","khaki")

# no c("orange","yellow","blue","red")

png("R_images/publication_tripartite_network.png", width=6000, height=4000, res=300)
plotweb(web.dc, method="normal", labsize=2, y.lim=c(-0.5,2.7), x.lim=c(0,1.25), adj.high=c(0,-0.5), adj.low=c(0.5,-0.1), y.width.low=0.05, y.width.high=0.1, high.y=2.5, low.y=1.54, low.spacing=0.063, arrow="down", col.low="transparent", bor.col.low="transparent", col.interaction=c("orange","yellow","blue","red"), low.plot=T, empty=F)
plotweb(web.ha, method="normal", labsize=2,  text.rot=90, y.width.low=0.1, y.width.high=0.09,	high.y=1.5, low.y=0.5, low.lab.dis=0.05, high.lablength=0, high.spacing=0.085, low.spacing=0.02, col.high="transparent",  col.interaction=c("orange","yellow","blue","red"), col.low=col.prey, add=T, empty=F)
legend("bottom", legend=name.multi, fill=col.multi, bty="n", border="black",ncol=6, xpd=T, cex=1.5)
dev.off()
```
...
melt.comms$order = "NA"
melt.comms[grep("Acanthosoma|Anthocoris|Coreus|Deraeocoris|Elasmucha|Neolygus|Palomena|Pentatoma|Psallus", melt.comms$variable),"order"] = "Hemiptera - Heteroptera"
melt.comms[grep("Agathomyia|Chironomus|Lonchoptera|Schwenckfeldina", melt.comms$variable), "order"] = "Diptera"
melt.comms[grep("Agriopis|Amphipyra|Bucculatrix|Cameraria|Orthosia|Pandemis|Tiliacea", melt.comms$variable), "order"] = "Lepidoptera"
melt.comms[grep("Anelosimus|Clubiona|Hylyphantes|Metellina|Oligolophus|Paidiscura|Paroligolophus|Philodromus|Tenuiphantes|Tetragnatha|Theridion", melt.comms$variable), "order"] = "Arachnida"
melt.comms[grep("Aphthona|Cantharis|Enicmus|Phyllopertha|Polydrusus|Rhagonycha|Rhyzobius", melt.comms$variable), "order"] = "Coleoptera"
melt.comms[grep("Cacopsylla|Callipterinella|Drepanosiphum|Eucallipterus|Euceraphis|Macrosiphum|Psyllopsis|Rhopalosiphum", melt.comms$variable), "order"] = "Hemiptera - Aphidomorpha"
melt.comms[grep("Cepaea", melt.comms$variable), "order"] = "Gastropoda"
melt.comms[grep("Chrysoperla|Hemerobius", melt.comms$variable), "order"] = "Neuroptera"
melt.comms[grep("Entomobrya|Sminthurus", melt.comms$variable), "order"] = "Collembola"
melt.comms[grep("Glyphotaelius", melt.comms$variable), "order"] = "Trichoptera"
melt.comms[grep("Lasius|Pristiphora|Trioxys", melt.comms$variable), "order"] = "Hymenoptera"
melt.comms[grep("Panorpa", melt.comms$variable), "order"] = "Mecoptera"
melt.comms[grep("Tachypodoiulus", melt.comms$variable), "order"] = "Myriapoda"
melt.comms[grep("Ectopsocus|Graphopsocus|Valenzuela", melt.comms$variable), "order"] = "Psocoptera"
...
pa = ggplot(melt.comms, aes(time, value)) +
...
pb = ggplot(melt.guts, aes(time, value)) +
  geom_bar(stat="identity", position="fill", aes(fill=variable)) +
  scale_fill_manual(values = color16, name = "") +
  facet_grid(.~county, scales="free") + 
  theme(legend.position = "right", axis.text = element_text(size = 15), 
