...
mean(no.blk.ratio$Harmonia_axyridis)*100
sd(no.blk.ratio$Harmonia_axyridis)/sqrt(length(no.blk.ratio$Harmonia_axyridis))*100

mean(blk.ratio$Harmonia_axyridis)*100
sd(blk.ratio$Harmonia_axyridis)/sqrt(length(blk.ratio$Harmonia_axyridis))*100

# mean with se of preys
mean(rowSums(no.blk.ratio[,!(colnames(no.blk.ratio) %in% c("Harmonia_axyridis","sample_id","sample_type","pcr","time","site"))]))*100
sd(rowSums(no.blk.ratio[,!(colnames(no.blk.ratio) %in% c("Harmonia_axyridis","sample_id","sample_type","pcr","time","site"))]))/sqrt(length(rowSums(no.blk.ratio[,!(colnames(no.blk.ratio) %in% c("Harmonia_axyridis","sample_id","sample_type","pcr","time","site"))])))*100

mean(rowSums(blk.ratio[,!(colnames(blk.ratio) %in% c("Harmonia_axyridis","sample_id","sample_type","pcr","time","site"))]))*100
sd(rowSums(blk.ratio[,!(colnames(blk.ratio) %in% c("Harmonia_axyridis","sample_id","sample_type","pcr","time","site"))]))/sqrt(length(rowSums(blk.ratio[,!(colnames(blk.ratio) %in% c("Harmonia_axyridis","sample_id","sample_type","pcr","time","site"))])))*100
```
...
norm.no.blk.reads = norm.no.blk.reads[(rownames(norm.no.blk.reads) %in% c(rownames(norm.blk.reads))),]

# Ordering by rownames
norm.no.blk.reads = norm.no.blk.reads[order(rownames(norm.no.blk.reads)),]
norm.blk.reads = norm.blk.reads[order(rownames(norm.blk.reads)),]
```


```{r Mann-Whitney test on blocking primers}
norm.no.blk.reads$Total_preys = rowSums(norm.no.blk.reads[,c(colnames(norm.no.blk.reads) != "Harmonia_axyridis")])
norm.blk.reads$Total_preys = rowSums(norm.blk.reads[,c(colnames(norm.blk.reads) != "Harmonia_axyridis")])

wilcox.test(norm.no.blk.reads$Harmonia_axyridis, norm.blk.reads$Harmonia_axyridis, paired=T, exact=F)
wilcox.test(norm.no.blk.reads$Total_preys, norm.blk.reads$Total_preys, paired=T, exact=F)
```
...
niche.ratio[,!c(colnames(niche.ratio) %in% c("sample_id","sample_type","pcr","site","time","county"))][niche.ratio[,!c(colnames(niche.ratio) %in% c("sample_id","sample_type","pcr","site","time","county"))] > 0] <- 1
