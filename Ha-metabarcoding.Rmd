...
pack.list = c("aod","bipartite","cowplot","devtools","dplyr","EcoSimR","ggplot2","gplots","gridExtra","iNEXT","lattice","lme4","lmtest","MASS","mgcv","network","plyr","reshape2","stringi","UpSetR","vegan")
...
box_a = ggplot(full_blk_melt,(aes(specie_type, value))) +
  ...
  theme_bw() +
  facet_grid(.~pcr) +
  theme(legend.position="NA", axis.text.x=element_blank(), axis.ticks.x=element_blank(), axis.text=element_text(size=15), axis.title=element_text(size=15), strip.text=element_text(size=15))
...
box_b = ggplot(full_blk_melt,(aes(specie_type, log10(value)))) +
  ...
  theme_bw() +
  facet_grid(.~pcr) +
  theme(legend.position="right", legend.title=element_blank(), legend.text=element_text(size=15), axis.text.x=element_blank(), axis.ticks.x=element_blank(), axis.text=element_text(size=15), axis.title=element_text(size=15), strip.text=element_text(size=15))

png("R_images/boxplot_blk_vs_no-blk.png", height=4000, width=4000, units="px", res=300)
cow_plot(box_a, box_b, label=c("a)","b)"), ncol=2, nrow=1)
dev.off()
```
...
# NOTE: 
to change niche null model into diet breadth...also...is it needed? What's the purpose of niche breadth?
