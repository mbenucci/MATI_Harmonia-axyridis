...
adonis(dist.sp.com~fact_county*fact_time, permutations = 10000, method="jaccard")
```
...
plot(meta.comms<-metaMDS(dist.sp.com, k=2, trymax=200, method="jaccard"),type="n",ylim=c(-0.7,0.4), xlim=c(-0.6,0.6))
...
meta.guts.comms = full_join(ratio.guts, ratio.sp.comms)

# to sort out!!!! (values > 0 <- 1)

#meta.guts.comms

adonis.guts.comms = meta.guts.comms[,!c(colnames(meta.guts.comms) %in% c("sample_id","sample_type","pcr","site","time","county"))]

fact_county = matrix(c(meta.guts.comms$county),ncol=1) 
fact_time = matrix(c(meta.guts.comms$time), ncol=1)
fact_type = matrix(c(meta.guts.comms$sample_type), ncol=1)

dist.guts.comms = vegdist(adonis.guts.comms)

adonis(dist.guts.comms~fact_county*fact_time*fact_type, permutations = 10000, method="jaccard")
```
