...
#rownames(web.ha)
#gsub("[a-z(?:Trombidiformes)]{5,} ",".", rownames(web.ha))
```

#NOTE 2:
Change order of network to match with parasitoid and Harmonia!!!

```{r tripartite}
#png("R_images/test_network.png", width=4000, height=4000, res=300)
plotweb(web.ha, method="normal", labsize=2, y.width.low=0.1, y.width.high=0.08,	high.y=1.5, low.y=0.5, low.lab.dis=0.05, high.lab.dis=0.1, high.spacing=0.08, low.spacing=0.02, adj.high=c(0.5,1.7), y.lim=c(0,3), col.high="white",  col.interaction=c("orange","yellow","blue","red"))
plotweb(web.dc, method="normal", labsize=2, y.width.low=0.05, y.width.high=0.1, high.y=2.5, low.y=1.54, low.lab.dis=0, low.spacing=0.058, adj.low=c(0.5,1.1), arrow="down", col.interaction=c("orange","yellow","blue","red"), low.plot=F, add=T, empty=F)
#legend("bottom", legend=name.multi, fill=col.multi, bty="n", border="black",ncol=4, xpd=T)
#dev.off()
```
```{r}
plotweb(web.dc, method="normal", labsize=2, y.lim=c(-1,2.7), x.lim=c(0,1.25), adj.low=c(0.5,-0.1), y.width.low=0.05, y.width.high=0.1, high.y=2.5, low.y=1.54, low.spacing=0.063, arrow="down", col.low="transparent", bor.col.low="transparent", col.interaction=c("orange","yellow","blue","red"), low.plot=T, empty=F)
plotweb(web.ha, method="normal", labsize=2,  text.rot=90, y.width.low=0.1, y.width.high=0.08,	high.y=1.5, low.y=0.5, low.lab.dis=0.05, high.lablength=0, high.spacing=0.085, low.spacing=0.02, col.high="transparent",  col.interaction=c("orange","yellow","blue","red"), add=T, empty=F)
```
