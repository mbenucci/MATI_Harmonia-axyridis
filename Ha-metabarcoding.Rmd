...
wilcox.test(rowSums(norm.no.blk.reads[,1:4]), rowSums(norm.blk.reads[,1:4]))
```

```{r calculation mean ratio with standard error}
norm.no.blk.ratio = norm.no.blk.reads[,]/rowSums(norm.no.blk.reads)
norm.blk.ratio = norm.blk.reads[,]/rowSums(norm.blk.reads)

# mean with se of Harmonia
mean(norm.no.blk.ratio$Harmonia_axyridis)*100
sd(norm.no.blk.ratio$Harmonia_axyridis)/sqrt(length(norm.no.blk.ratio$Harmonia_axyridis))*100

mean(norm.blk.ratio$Harmonia_axyridis)*100
sd(norm.blk.ratio$Harmonia_axyridis)/sqrt(length(norm.blk.ratio$Harmonia_axyridis))*100

# mean with se of preys
mean(rowSums(norm.no.blk.ratio[,!(colnames(norm.no.blk.ratio) %in% c("Harmonia_axyridis"))]))*100
sd(rowSums(norm.no.blk.ratio[,!(colnames(norm.no.blk.ratio) %in% c("Harmonia_axyridis"))]))/sqrt(length(rowSums(norm.no.blk.ratio[,!(colnames(norm.no.blk.ratio) %in% c("Harmonia_axyridis"))])))*100

mean(rowSums(norm.blk.ratio[,!(colnames(norm.blk.ratio) %in% c("Harmonia_axyridis"))]))*100
sd(rowSums(norm.blk.ratio[,!(colnames(norm.blk.ratio) %in% c("Harmonia_axyridis"))]))/sqrt(length(rowSums(norm.blk.ratio[,!(colnames(norm.blk.ratio) %in% c("Harmonia_axyridis"))])))*100
```
...
Visually checking the samples showed that 5 samples sequenced with blocking primers didn't pass the QC. In order to run the Wilcoxon rank sign test with paired samples we need to remove the extra samples from the dataset without blocking primers and order the samples.

```{r removing same lenght of samples}
# selecting samples that are present in both datasets only
norm.no.blk.reads2 = norm.no.blk.reads[(rownames(norm.no.blk.reads) %in% c(rownames(norm.blk.reads))),]

# Ordering by rownames
norm.no.blk.reads2 = norm.no.blk.reads2[order(rownames(norm.no.blk.reads2)),]
norm.blk.reads = norm.blk.reads[order(rownames(norm.blk.reads)),]
```


```{r Wilcoxon rank test on blocking primers}
norm.no.blk.reads2$Total_preys = rowSums(norm.no.blk.reads2[,c(colnames(norm.no.blk.reads2) != "Harmonia_axyridis")])
norm.blk.reads$Total_preys = rowSums(norm.blk.reads[,c(colnames(norm.blk.reads) != "Harmonia_axyridis")])

wilcox.test(norm.no.blk.reads2$Harmonia_axyridis, norm.blk.reads$Harmonia_axyridis, paired=T, exact=F)
wilcox.test(norm.no.blk.reads2$Total_preys, norm.blk.reads$Total_preys, paired=T, exact=F)
```

We also run the Wilcoxon rank sign test on the proportions between predator and prey in paired samples with and without blocking primers.

```{r Wilcoxon rank test proportions blocking primers}
wilc.no.blk.ratio = norm.no.blk.ratio
wilc.blk.ratio = norm.blk.ratio

wilc.no.blk.ratio = wilc.no.blk.ratio[c(rownames(wilc.no.blk.ratio) %in% rownames(wilc.blk.ratio)),]

wilcox.test(wilc.no.blk.ratio$Harmonia_axyridis , wilc.blk.ratio$Harmonia_axyridis, paired=T, exact=F)

wilcox.test(rowSums(wilc.no.blk.ratio[,c(colnames(wilc.no.blk.ratio)!="Harmonia_axyridis")]), rowSums(wilc.blk.ratio[,c(colnames(wilc.blk.ratio)!="Harmonia_axyridis")]), paired=T, exact=F)
```
...
```{r proportion data frame for plot}
ratio_no_blk = as.data.frame(t(wilc.no.blk.ratio))
ratio_blk = as.data.frame(t(wilc.blk.ratio))

ratio_no_blk$pcr = "no-blocking primers"
ratio_blk$pcr = "blocking primers"

ratio_no_blk$specie_type = "NA"
ratio_no_blk[grep("Harmonia_axyridis", rownames(ratio_no_blk)),"specie_type"] = "predator"
ratio_no_blk[grep("Harmonia_axyridis", rownames(ratio_no_blk), invert = T), "specie_type"] = "prey"

ratio_blk$specie_type = "NA"
ratio_blk[grep("Harmonia_axyridis", rownames(ratio_blk)),"specie_type"] = "predator"
ratio_blk[grep("Harmonia_axyridis", rownames(ratio_blk), invert = T), "specie_type"] = "prey"

ratio_no_blk$specie = rownames(ratio_no_blk)
ratio_blk$specie = rownames(ratio_blk)

ratio_no_blk[ratio_no_blk == 0] <- NA
ratio_blk[ratio_blk == 0] <- NA

full_ratio = full_join(ratio_no_blk, ratio_blk)

full_ratio = melt(full_ratio)
```
...
full_ratio$pcr = factor(full_ratio$pcr, levels=c("no-blocking primers","blocking primers"))
box_b = ggplot(full_ratio,(aes(specie_type, value))) +
  geom_boxplot(aes(fill=specie_type), outlier.shape = NA) +
  geom_jitter(alpha=0.5, size=4) +
  labs(x="", y="Predator/prey DNA ratio") +
  theme_bw() +
  facet_grid(.~pcr) +
...
```{r}
ggplot(full_blk_melt, aes(pcr, value)) +
  geom_bar(stat="identity", position="fill",aes(fill=specie)) +
  labs(x="", y="Normalised DNA reads") +
  theme_bw() +
  theme(legend.position="none", axis.text.x=element_blank(), axis.ticks.x=element_blank(), axis.text=element_text(size=15), axis.title=element_text(size=15), strip.text=element_text(size=15), plot.margin=unit(c(0,0.25,0,0.25), unit="cm"))
...
We detected a total of 16 interactions in Oxfordshire in May (N tot=63), only 10 interactions in Oxfordshire in October (N tot=59); while we detected 24 and 18 interactions respectively in Yorkshire in May (N tot=104) and October (N tot=99).
...
```{r setting frames for permanova of guts reads}
...
ratio.guts = subset(ratio.guts, subset=rowSums(ratio.guts[,!c(colnames(ratio.guts) %in% c("sample_id","sample_type","pcr","site","time","county"))]) > 0)

# splitting potential preys from parasitoids
ratio.prey = ratio.guts[,!c(colnames(ratio.guts) %in% c("Dinocampus_coccinellae"))]
ratio.paras = ratio.guts[,c(colnames(ratio.guts) %in% c("sample_id","sample_type","pcr","site","time","county","Dinocampus_coccinellae"))]

rownames(ratio.prey) = ratio.prey$sample_id
rownames(ratio.paras) = ratio.paras$sample_id
```

```{r running permanova}
colnames(ratio.prey)
matrix.ratio.guts = ratio.prey[,!c(colnames(ratio.prey) %in% c("sample_id","sample_type","pcr","site","time","county"))]
matrix.ratio.guts[,][matrix.ratio.guts[] > 0] = 1

fact_county.g = matrix(c(ratio.prey$county), ncol=1) 
fact_time.g = matrix(c(ratio.prey$time), ncol=1)
fact_all.g = matrix(c(ratio.prey$county, ratio.prey$time), ncol=2)
...
abund.guts = as.data.frame(ratio.prey)
abund.guts[,!c(colnames(abund.guts) %in% c("sample_id","sample_type","pcr","site","time","county"))][abund.guts[,!c(colnames(abund.guts) %in% c("sample_id","sample_type","pcr","site","time","county"))] > 0] <- 1

abund.paras = as.data.frame(ratio.paras)
abund.paras[,!c(colnames(abund.paras) %in% c("sample_id","sample_type","pcr","site","time","county"))][abund.paras[,!c(colnames(abund.paras) %in% c("sample_id","sample_type","pcr","site","time","county"))] > 0] <- 1


raref.guts = as.data.frame(abund.guts[,!c(colnames(abund.guts) %in% c("sample_id","sample_type","pcr","site"))] %>% group_by(time, county) %>% summarise_all(funs(sum)))

raref.guts = t(raref.guts)
...
network.full = full_join(abund.guts, abund.paras)

network.guts = as.data.frame(network.full[,!c(colnames(network.full) %in% c("sample_id","sample_type","pcr","site"))] %>% group_by(time, county) %>% summarise_all(funs(sum)))

network.guts$county = gsub("rd{1}|[a-z]{5}$", "", network.guts$county)
rownames(network.guts) = paste(network.guts$county, network.guts$time)

network.guts = network.guts[,!c(colnames(network.guts) %in% c("time","county"))]
...
```{r order}
#"Betulaphis","Clethrobius_comes","Taeniothrips_inconsequens","Thripidae",
web.ha = web.ha[c("Callipterinella","Trombidiformes_sp._BOLD:ACI3657","Euceraphis","Ocytata_pallipes","Drepanosiphum_platanoidis","Chrysomelidae","Chironomus_nuditarsis","Ectopsocus_californicus","Eucallipterus_tiliae","Psocoptera"),]
...
guts.summary = as.data.frame(abund.guts[,!c(colnames(abund.guts) %in% c("sample_id"))] %>% group_by(sample_type,time,county,site,pcr) %>% summarise_all(list(sum)))
...
full_ratio$pcr = factor(full_ratio$pcr, levels=c("no-blocking primers","blocking primers"))
blk_b = ggplot(full_ratio,(aes(specie_type, value))) +
  geom_boxplot(aes(fill=specie_type), outlier.shape = NA) +
  geom_jitter(alpha=0.5, size=4) +
  labs(x="", y="Predator/prey DNA ratio") +
  theme_bw() +
  facet_grid(.~pcr) +
  theme(legend.position="right", legend.title=element_blank(), legend.text=element_text(size=20), axis.text.x=element_blank(), axis.ticks.x=element_blank(), axis.text=element_text(size=15), axis.title=element_text(size=15), strip.text=element_text(size=15), plot.margin=unit(c(0,0,0,0.25), unit="cm"))
...
heat_a<-ggplot(heat_blk_melt[heat_blk_melt$specie %in% c("D.platanoidis","P.viburni"),],(aes(pcr, variable))) +
  ...
  theme_classic() +
  facet_grid(.~specie) + labs(x="", y="") +
  theme(legend.position="right", legend.text=element_text(size=15), axis.text.x=element_text(size=15), axis.ticks.x=element_blank(), axis.text=element_text(size=15), axis.title=element_text(size=15), strip.text=element_text(size=12), plot.margin=unit(c(0,-0.2,0,0.8), unit="cm"))
...
heat_b<-ggplot(heat_blk_melt[heat_blk_melt$specie %in% c("Chrysomelidae","E.betulae"),],(aes(pcr, variable))) +
  ...
  theme_classic() +
  facet_grid(.~specie) + labs(x="", y="") +
  theme(legend.position="right", legend.text=element_text(size=15), axis.text.x=element_text(size=15), axis.ticks.x=element_blank(), axis.text.y = element_blank(), axis.text=element_text(size=15), axis.title=element_text(size=15), strip.text=element_text(size=12), plot.margin=unit(c(0,-0.2,0,0.8), unit="cm"))
...
heat_c<-ggplot(heat_blk_melt[heat_blk_melt$specie_type %in% "predator",],(aes(pcr, variable))) +
  ...
  theme_classic() +
  facet_grid(.~specie) + labs(x="", y="") +
  theme(legend.position="right", legend.text=element_text(size=15), axis.text.x=element_text(size=15), axis.ticks.x=element_blank(), axis.text.y = element_blank(), axis.text=element_text(size=15), axis.title=element_text(size=15), strip.text=element_text(size=12), plot.margin=unit(c(0,0,0,1), unit="cm"))
...
plot_grid(heat_a, heat_b, heat_c, labels=c("a)","b)","c)"), nrow=1, label_size=25, label_fontface=1)
dev.off()
```
...
ordispider(meta.guts,group=fact_county.g[,1],col=c("black", "blue"), label=T, cex=1.5)
ordiellipse(meta.guts,group=fact_time.g[,1],kind="ehull", lty=c(1,3),label=F,lwd=2)
...
ordispider(meta.comms,group=fact_all[,1],col=c("black", "blue"),label=T, cex=1.5)
ordiellipse(meta.comms,group=fact_all[,2],kind="ehull", lty=c(1,3),label=F, lwd=2)
...
ordispider(meta.guts.comms,group=fact_type[,1],col=c("black", "blue"),label=F, cex=1.5)
...
ordispider(meta.guts.comms,group=fact_type[,1],col=c("black", "blue"),label=T, cex=1.5)
ordiellipse(meta.guts.comms,group=fact_time[,1],kind="ehull",lty=c(1,3),label=F, lwd=2)
points(meta.guts.comms, select=which(fact_county[,1]=="Oxfordshire" & fact_time[,1]=="May"),col="black", pch=1, cex=2)
points(meta.guts.comms, select=which(fact_county[,1]=="Yorkshire" & fact_time[,1]=="May"),col="blue", pch=22, cex=2)
points(meta.guts.comms, select=which(fact_county[,1]=="Oxfordshire" & fact_time[,1]=="Oct"),col="black", pch=16, cex=2)
...
text(0.375,2.3, "*", cex=3)
text(0.7,2.3, paste("N =", web.dc[3,1]), cex=1.5, col="white")
# web.ha interactions
text(0.01,1.2, "*", cex=3)
text(0.035,1.2, "*", cex=3)
text(0.15,1.2, paste("N =", web.ha[3,1]), cex=1.5) 
text(0.5,1.2, paste("N =", web.ha[3,2]), cex=1.5)
text(0.745,1.2, "*", cex=3)
text(0.772,1.2, "*", cex=3)
text(0.817,1.2, "*", cex=3, col="white")
text(0.91,1.2, paste("N =", web.ha[3,4]), cex=1.5)
text(1.02,1.2, "*", cex=3)
text(1.05,1.2, "*", cex=3)
text(1.09,1.2, paste("N =", web.ha[8,4]), cex=1.5)
text(1.135,1.2, "*", cex=3)
text(1.2,1.2, paste("N =", web.ha[10,4]), cex=1.5)
