...
```{r blk vs no-blk reads boxplot}
norm.no.blk.reads$sample = "no blocking primers"
norm.blk.reads$sample = "with blocking primers"

melt.no.blk = melt(norm.no.blk.reads)
melt.blk = melt(norm.blk.reads)

full.norm.reads = rbind(melt.no.blk, melt.blk)

full.norm.reads$type = "NA"
full.norm.reads[full.norm.reads$variable == "Harmonia_axyridis","type"] = "predator"
full.norm.reads[full.norm.reads$variable != "Harmonia_axyridis","type"] = "prey"

ggplot(full.norm.reads[full.norm.reads$value > 0,], aes(sample, value)) +
  geom_boxplot(outlier.shape=NA) +
  geom_jitter(alpha=0.4) +
  facet_grid(type~., scales = "free") +
  theme_bw() +
  labs(x="", y="Normalised DNA reads (count)")
```
...
#grid.arrange(arrangeGrob(a, c, nrow=2), arrangeGrob(b, ncol=1), ncol=2, left="Sample ID")
dev.off()
```
