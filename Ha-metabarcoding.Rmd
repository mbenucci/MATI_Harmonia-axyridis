...
#colnames(web.dc) = paste(colnames(web.dc),"( N =",sum(web.dc),")")
#rownames(web.dc)[1] = paste(rownames(web.dc)[1],"( N =",length(oxf.guts[grep("may", oxf.guts$sample_id),"sample_id"]),")")
#rownames(web.dc)[2] = paste(rownames(web.dc)[2],"( N =",length(yor.guts[grep("may", yor.guts$sample_id),"sample_id"]),")")
#rownames(web.dc)[3] = paste(rownames(web.dc)[3],"( N =",length(oxf.guts[grep("oct", oxf.guts$sample_id),"sample_id"]),")")
# rownames(web.dc)[4] = paste(rownames(web.dc)[4],"( N =",length(yor.guts[grep("oct", yor.guts$sample_id),"sample_id"]),")")

png("R_images/publication_tripartite_network.png", width=6000, height=4000, res=300)
plotweb(web.dc, method="normal", labsize=3, y.lim=c(-0.5,2.7), x.lim=c(0,1.25), adj.high=c(0,-0.5), adj.low=c(0.5,-0.1), y.width.low=0.05, y.width.high=0.1, high.y=2.5, low.y=1.54, low.spacing=0.077, arrow="down", col.low="transparent", bor.col.low="black", col.interaction=c("orange","yellow","blue","red"), low.plot=T, empty=F)
