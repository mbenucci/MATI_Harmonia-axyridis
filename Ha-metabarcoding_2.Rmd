...
## Input files and first overview

We load the 3 different files into three different data frames
...
# Calculating contamination threshold
## First data frame
...
plotweb(web.dc, y.width.low=0.05, y.width.high=0.1, low.y=1.5, high.y=2.5, low.lab.dis=0, low.spacing=0.05, adj.low=c(0.5,1.1), low.plot=F, arrow="down", add=T, empty=F, col.interaction=c("orange","yellow","blue","red"))
```


## TO CHECK ABUNDANCE OF COMMUNITY AND SHARED TAXA ACROSS COMMS AND GUTS!

```{r tripartite with abundance comms test, include=F}

#abund.guts
#test.web = as.data.frame(abund.comms)
#rownames(test.web) = c("Oxf_May-Ha","York_May-Ha","Oxf_Oct-Ha","York_Oct-Ha")
#test.web = test.web[,-c(1:2)]
#test.web = t(test.web)

# = as.vector(test.web)
#rownames(test.web)

#web1 = abund.guts[!c(rownames(abund.guts) %in% "Dinocampus_coccinellae"),]
#web1 = web1[!c(rownames(web1)%in%"Hemiptera"),]

#web2 = abund.guts[c(rownames(abund.guts) %in% "Dinocampus_coccinellae"),]
...
#web2 = as.data.frame(t(web2))
#web2[,1] = as.integer(web2[,1])

#web1 = data.matrix(web1)
#web2 = data.matrix(web2)

#plotweb(web1, y.width.low=0.1, y.width.high=0.05,	y.lim=c(0,3), adj.high=c(0.5,1.5), high.lab.dis=0, low.lablength=5, col.high="white", low.lab.dis=0.03, low.spacing=0.02, low.abun = test.web)
#plotweb(web2, y.width.low=0.05, y.width.high=0.1, low.y=1.5, high.y=2.5, low.lab.dis=0, low.spacing=0.166, adj.low=c(0.5,1.1), low.plot=FALSE, arrow="down", add=TRUE)
```
...
```{r ggnetwork}
### NEED TO CREATE THE NETWORK FROM THE DATA FRAMES
#test = as.data.frame(ratio.guts)
#test[,1:15][test[,1:15] > 0] <- 1

#test_1 = as.data.frame(test[,!c(colnames(test) %in% c("pcr","sample_type","sample_id"))] %>% group_by(site,time,county) %>% summarise_all(funs(sum)))

#test_1 = test[,!c(colnames(test) %in% c("pcr","sample_type","sample_id"))]

#str(test_1)
#colnames(test_1)
#nrow(test_1)
#length(test_1$site)
#length(test_1$county)
#length(test_1$time)

#info_site = test_1$site
#info_time = test_1$time
#info_county = test_1$county
#info_weight = c(test_1[1:nrow(test_1),4:ncol(test_1)])

#rownames(test_1) = paste(test_1$site, test_1$time, sep="_")

#test_1 = test_1[,!c(colnames(test_1) %in% c("site","time","county"))]

#str(as.matrix(test_1))
#test <- network(as.matrix(test_1),directed=T)
#ggnetwork(test)

#test %v% "time" <- info_time
#test %v% "county" <- info_county
#test %v% "site" <- info_site
#test %v% "importance" <- info_weight
#ggnetwork(test)

#e <- network.edgecount(test)
#set.edge.attribute(test, "time", test_1$time, e)
#set.edge.attribute(test, "site", test_1$site)
#set.edge.attribute(test, "county", test_1$county)

#ggnetwork(test, layout = "fruchtermanreingold", cell.jitter = 0.75)
#ggnetwork(test, layout = "target", niter = 100)

#ggplot(test, aes(x=x,y=y,xend=xend,yend=yend)) +
#  geom_edges() +
#  geom_nodes() +
#  geom_nodelabel(aes(label = vertex.names)) +
#  facet_grid(time~county)
